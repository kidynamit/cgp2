...
public:
    ...
    ~OpNode() {
		try {
			...
			sln = dynamic_cast<ShapeNode*>(left);
			oln = dynamic_cast<OpNode*>(left);
			if (sln != nullptr) delete sln;
			else if (oln != nullptr) delete oln;
			else { std::cerr << "Error: @~OpNode !ShapeNode and !OpNode" << std::endl; }	
			
			srn = dynamic_cast<ShapeNode*>(right);
			orn = dynamic_cast<OpNode*>(right);
			if (srn != nullptr) delete srn;
			else if (orn != nullptr) delete orn;
			else { std::cerr << "Error: @~OpNode !ShapeNode and !OpNode" << std::endl; }
		} catch (exception & e) {
			std::cerr << "Exception encountered @~OpNode()\n\t" << e.what () << std::endl; 
		} 
	}
};
...
private:
	...
	void storeShapeNodes(SceneNode * scn, std::vector<ShapeNode*> & vShapeNodes);
public:
